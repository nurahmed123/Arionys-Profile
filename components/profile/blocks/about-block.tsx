...
export function AboutBlock({ block, theme = "default" }: AboutBlockProps) {
  const { content, descriptionType = "plain" } = block.content || {}
      ...
      <CardContent className="p-6">
        <div className="flex items-start space-x-4">
          ...
          <div className="flex-1">
            {block.title && <h3 className={`${getTextClasses({theme, type: "heading", size: "lg"})} mb-3 text-balance`}>{block.title}</h3>}
            <div className="prose prose-sm dark:prose-invert max-w-none">
              {descriptionType === "markdown" ? (
                ...
                <p className={`${getTextClasses({theme, type: "body"})} leading-relaxed text-pretty`}>{content}</p>
              )}
            </div>
          </div>
        </div>
